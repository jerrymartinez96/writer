...
const ConsistencyToolRoom = () => {
    ...
    const buildDocuments = async () => {
        ...
        return [
            ...
            ...(includeWorld ? worldItems.map((item) => ({ ...item, type: 'worldItem', title: item.title })) : []),
    ...
    const applyFinding = async (finding) => {
        if (!finding.originalText) return;
        if (!finding.replacementText.trim() && !finding.replacementEdited) {
            setError('La IA no propuso una reescritura segura. Edita el reemplazo o vuelve a ejecutar la auditoría antes de aplicar.');
            return;
        }
        ...
        if (nextContent === null) {
            setError('El texto original ya no coincide. Actualiza el análisis antes de aplicar el cambio.');
            return;
        }
        try {
            ...
            setError(applyError?.message || 'No se pudo aplicar el cambio.');
        }
    };
    ...
    return <ToolRoomShell room={room} status={isLoading ? 'processing' : result ? 'saved' : 'ready'} context={<div className="flex items-center gap-3"><ShieldCheck size={17} className="text-emerald-500" /><span>Auditoría global sobre {scope === 'all' ? 'toda la obra' : SCOPES.find((item) => item[0] === scope)?.[1].toLowerCase()}</span></div>}>
        <div className="mx-auto max-w-5xl space-y-5">
            ...
            {result && <section className="rounded-3xl border border-[var(--border-main)] bg-[var(--bg-editor)] p-6 lg:p-8"><div className="flex items-start gap-3 rounded-2xl border border-orange-500/20 bg-orange-500/5 p-4"><FileSearch size={18} className="mt-0.5 shrink-0 text-orange-500" /><div><p className="text-sm font-black">{result.findings.length ? `${result.findings.length} resultado(s) para revisar` : 'No se encontraron detalles para revisar'}</p><p className="mt-1 text-xs leading-relaxed text-[var(--text-muted)]">{result.summary || 'La auditoría terminó sin un resumen adicional.'}</p></div></div><div className="mt-5 space-y-3">{result.findings.map((finding) => { const target = documents.find((document) => document.id === finding.documentId); return <article key={finding.id} className={`rounded-2xl border p-4 ${finding.status === 'applied' ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-[var(--border-main)] bg-[var(--bg-app)]'}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-black">{finding.title}</p><p className="mt-1 text-xs text-[var(--text-muted)]">{target?.title || finding.documentId} · {finding.category} · confianza {Math.round(finding.confidence * 100)}%</p></div><span className={`rounded-full px-2 py-1 text-[9px] font-black uppercase ${finding.status === 'applied' ? 'bg-emerald-500/10 text-emerald-600' : severityClass[finding.severity]}`}>{finding.status === 'applied' ? 'Aplicado' : finding.severity}</span></div>{finding.excerpt && <p className="mt-3 rounded-xl border border-[var(--border-main)] bg-[var(--bg-editor)] p-3 text-xs leading-relaxed">{finding.excerpt}</p>}<p className="mt-3 text-xs leading-relaxed text-[var(--text-muted)]">{finding.reason}</p>{finding.originalText && finding.status !== 'applied' && <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2"><div><p className="text-[9px] font-black uppercase tracking-wider text-red-500">Encontrado</p><p className="mt-1 rounded-xl border border-red-500/20 bg-red-500/5 p-3 text-xs">{finding.originalText}</p></div><label className="text-[9px] font-black uppercase tracking-wider text-emerald-600">Reemplazo (vacío = eliminar)<textarea value={finding.replacementText} onChange={(event) => updateReplacement(finding.id, event.target.value)} rows={2} className="mt-1 w-full resize-none rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 text-xs font-normal normal-case outline-none focus:border-emerald-500" /></label></div>}<div className="mt-4">{finding.status === 'applied' ? <span className="inline-flex items-center gap-2 text-xs font-black text-emerald-600"><Check size={14} /> Cambio guardado con snapshot</span> : finding.originalText ? <button type="button" onClick={() => applyFinding(finding)} className="inline-flex items-center gap-2 rounded-xl bg-orange-600 px-3 py-2 text-xs font-black text-white hover:bg-orange-500"><Check size={14} /> Aplicar cambio</button> : <span className="text-xs text-[var(--text-muted)]">Resultado informativo; no hay un cambio automático propuesto.</span>}</div></article>; })}</div></section>}
        </div>
    </ToolRoomShell>;
};
