...
const AUDIT_TYPES = [
    ['continuity', 'Continuidad y contradicciones'],
...
const CANON_CATEGORIES = new Set(['continuity', 'character', 'world', 'terminology', 'timeline', 'fact', 'rule', 'reference']);

const requiresGlobalReview = (finding, auditType) => finding.severity === 'high'
    || (Array.isArray(finding.documentIds) && finding.documentIds.length > 1)
    || CANON_CATEGORIES.has(finding.category)
    || ['continuity', 'character', 'world', 'terminology', 'timeline'].includes(auditType);
...
const AuditToolRoom = () => {
    ...
    const [auditType, setAuditType] = useState(storedProcess.auditType || 'continuity');
    ...
    const persistAudit = (patch = {}) => updateProcess('audit', {
        auditType,
        scope,
        query,
        canonical,
    ...
    const runAudit = async () => {
        ...
        setError('');
        setResult(null);
        persistAudit({ status: 'analyzing', error: null });
        try {
            ...
            setStatus('error');
            persistAudit({ status: 'failed', error: requestError?.message || 'No se pudo completar la auditoría.' });
        }
    };

    const applyFinding = async (finding) => {
        if (!finding.originalText || applyingFindingId) return;
        if (requiresGlobalReview(finding, auditType)) {
            setFindingErrors((previous) => ({ ...previous, [finding.id]: 'Este hallazgo afecta el canon o la continuidad. Debe revisarse en el Constructor Global.' }));
            return;
        }
        ...
        try {
            ...
            const nextResult = result ? { ...result, findings: result.findings.map((item) => item.id === finding.id ? { ...item, status: 'applied' } : item) } : result;
            setResult(nextResult);
            persistAudit({ status: 'completed', documents: documents.map((document) => document.id === target.id ? { ...document, content: nextContent } : document), result: nextResult });
        } catch (applyError) {
    ...
    const openFindingInConstructor = (finding) => {
        ...
        const prompt = `Revisa este hallazgo de Auditoría de obra antes de modificar el canon. Analiza la evidencia, genera alternativas y prepara un plan aprobado.\n\nTítulo: ${finding.title}\nCategoría: ${finding.category}\nSeveridad: ${finding.severity}\nConfianza: ${finding.confidence}\nMotivo: ${finding.reason}\nEvidencia: ${finding.excerpt || '(la auditoría no proporcionó un extracto adicional)'}\nTexto original señalado: ${finding.originalText || '(solo informativo)'}\nReemplazo sugerido: ${finding.replacementText || '(ninguno)'}`;
        try {
            window.sessionStorage.setItem('verne-ia-studio-launch', JSON.stringify({
                roomId: 'global-constructor',
                prompt,
                context,
                contextLabel: affectedDocuments.map((document) => document.title).join(', '),
                createdAt: new Date().toISOString(),
            }));
        } catch {
            setError('No se pudo preparar el hallazgo para el Constructor Global.');
            return;
        }
        openToolRoom('toolroom:global-constructor');
        persistAudit({ status: 'needs_review', activeFindingId: finding.id });
    };
    ...
    const updateReplacement = (findingId, replacementText) => {
        const nextResult = result ? { ...result, findings: result.findings.map((finding) => finding.id === findingId ? { ...finding, replacementText, replacementEdited: true } : finding) } : result;
        setResult(nextResult);
