...
const ChapterWriter = ({ state, updateRoomState, worldItems, chapters, characters, activeChapter, lazyLoadChapters, createChapter, selectChapter, setActiveView, onGoDesigner }) => {
    ...
    const sizeLabel = SIZE_OPTIONS.find((option) => option.id === size)?.detail || `${customMin}–${customMax} palabras`;
    ...
    const contextContent = useMemo(() => [
        ...
        `Escenas definidas:\n${JSON.stringify((planDraft || selectedPlan)?.scenes || [])}`,
        `Último capítulo:\n${lastChapter?.content || ''}`,
        `Información general:\n${worldItem?.content || ''}`,
        `Personajes:\n${characters.map((character) => `${character.name}: ${character.description || ''}`).join('\n')}`,
    ].join('\n\n'), [characters, lastChapter, planDraft, selectedPlan, structureContent, storedDesigner.writingStructure, worldItem]);

    const applyDraft = async (replacement, proposal) => {
        if (!planDraft || selectedExistingChapter) return;
        setStatus('saving');
        try {
            const target = selectedEmptyChapter || await createChapter({ title: planDraft.title, content: replacement, parentId: null, isVolume: false }, { preventRedirect: true });
            if (!target?.id) throw new Error('No se pudo crear el capítulo.');
            if (selectedEmptyChapter) await updateChapterContent(activeBook.id, target.id, replacement);
            await saveEntitySnapshot(activeBook.id, 'chapters', target.id, replacement, 'toolroom-cowriter-structure');
            await saveLocalSnapshot(target.id, replacement, 'toolroom-cowriter-structure');
    ...
    if (!selectedPlan) return <section className="mx-auto max-w-5xl rounded-3xl border border-[var(--border-main)] bg-[var(--bg-editor)] p-6 lg:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[9px] font-black uppercase tracking-[0.18em] text-emerald-600">Redacción guiada</p><h2 className="mt-2 text-3xl font-serif font-black">Elige un capítulo pendiente</h2><p className="mt-2 text-sm text-[var(--text-muted)]">Análisis guardado: {storedDesigner.analysisAnalyzedAt ? new Date(storedDesigner.analysisAnalyzedAt).toLocaleString('es-MX') : 'reciente'}.</p></div><button type="button" onClick={runAnalysis} disabled={busy} className="inline-flex items-center gap-2 rounded-xl border border-[var(--border-main)] px-3 py-2 text-xs font-black"><RefreshCw size={14} /> Actualizar análisis</button></div><div className="mt-6 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4 text-sm leading-relaxed">{analysis.summary || analysis.recommendation || 'Selecciona un capítulo definido en Estructura para comenzar.'}</div><div className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-2">{pendingChapters.map((chapter) => <button type="button" key={chapter.id} onClick={() => choosePlan(chapter)} className="rounded-2xl border border-[var(--border-main)] bg-[var(--bg-app)] p-4 text-left transition-colors hover:border-emerald-500"><div className="flex items-center justify-between gap-3"><p className="text-sm font-black">{chapter.position}. {chapter.title}</p><span className="rounded-full bg-amber-500/10 px-2 py-1 text-[9px] font-black uppercase text-amber-600">{chapter.status === 'empty' ? 'Vacío' : 'Pendiente'}</span></div><p className="mt-2 text-xs leading-relaxed text-[var(--text-muted)]">{chapter.summary || chapter.purpose || 'Revisar estructura del capítulo.'}</p></button>)}</div>{pendingChapters.length === 0 && <div className="mt-6 rounded-2xl border border-dashed border-[var(--border-main)] p-6 text-center text-sm text-[var(--text-muted)]">No se detectaron capítulos pendientes. Ve a “Diseñar próximo capítulo” para crear el siguiente.</div>}{error && <p className="mt-4 text-xs text-red-600">{error}</p>}</section>;
