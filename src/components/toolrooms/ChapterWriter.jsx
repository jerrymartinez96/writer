...
const ChapterWriter = ({ state, updateRoomState, worldItems, chapters, characters, activeChapter, lazyLoadChapters, createChapter, selectChapter, setActiveView, onGoDesigner }) => {
    ...
    const runAnalysis = async () => {
        ...
        try {
            ...
            setAnalysis(result);
            updateRoomState('creative-studio', { designer: { ...(state.designer || {}), analysis: result, analysisSourceHash: sourceHash, analysisAnalyzedAt: new Date().toISOString() } });
            setStatus('ready');
        } catch (requestError) {
