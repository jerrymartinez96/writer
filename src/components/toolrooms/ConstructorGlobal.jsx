...
import { buildMissionDocuments, MISSION_SCOPES, MISSION_TYPES, requestMissionAlternatives, requestMissionImpact, requestMissionOperations, requestMissionVerification } from '../../services/ai-next/MissionService';
...
const ImpactPanel = ({ impact, questions = [], onContinue }) => <section className="mt-5 rounded-2xl border border-cyan-500/25 bg-cyan-500/5 p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-[0.16em] text-cyan-600">Análisis de impacto</p><h3 className="mt-1 text-lg font-black">{impact.summary}</h3></div><span className="rounded-full bg-cyan-500/10 px-2.5 py-1 text-[10px] font-black uppercase text-cyan-700">Riesgo {impact.risk}</span></div><div className="mt-4 space-y-2">{impact.affectedDocuments.map((document) => <div key={document.documentId} className="flex items-start gap-3 rounded-xl border border-[var(--border-main)] bg-[var(--bg-editor)] p-3"><span className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${document.impact === 'high' ? 'bg-red-500' : document.impact === 'medium' ? 'bg-amber-500' : 'bg-emerald-500'}`} /><div><p className="text-xs font-black">{document.title} <span className="font-normal text-[var(--text-muted)]">· {document.action}</span></p><p className="mt-1 text-[11px] leading-relaxed text-[var(--text-muted)]">{document.reason}</p></div></div>)}</div>{(impact.warnings?.length || questions.length) > 0 && <div className="mt-4 rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-amber-700">{[...(impact.warnings || []), ...questions].map((item, index) => <p key={`${item}-${index}`}>• {item}</p>)}</div>}<button type="button" onClick={onContinue} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-cyan-600 px-3 py-2 text-xs font-black text-white hover:bg-cyan-500"><ChevronRight size={14} /> Generar alternativas</button></section>;

const AlternativeCards = ({ alternatives, selectedId, onSelect, onContinue }) => <section className="mt-5 rounded-2xl border border-violet-500/25 bg-violet-500/5 p-5"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-violet-600">Tres caminos posibles</p><h3 className="mt-1 text-lg font-black">Elige cómo quieres transformar la obra</h3><div className="mt-4 grid gap-3 lg:grid-cols-3">{alternatives.map((alternative) => <button type="button" key={alternative.id} onClick={() => onSelect(alternative.id)} className={`rounded-2xl border p-4 text-left ${selectedId === alternative.id ? 'border-violet-500 bg-violet-500/10' : 'border-[var(--border-main)] bg-[var(--bg-editor)]'}`}><span className="text-[9px] font-black uppercase tracking-wider text-violet-600">{alternative.type === 'conservative' ? 'Conservadora' : alternative.type === 'transformative' ? 'Transformadora' : 'Compensatoria'}</span><h4 className="mt-2 text-sm font-black">{alternative.title}</h4><p className="mt-2 text-xs leading-relaxed text-[var(--text-muted)]">{alternative.summary}</p><p className="mt-3 text-[10px] font-black uppercase text-[var(--text-muted)]">Cambios: {alternative.changes.length} · Riesgos: {alternative.risks.length}</p></button>)}</div>{selectedId && <button type="button" onClick={onContinue} className="mt-5 inline-flex items-center gap-2 rounded-xl bg-violet-600 px-3 py-2 text-xs font-black text-white hover:bg-violet-500"><ChevronRight size={14} /> Construir plan exacto</button>}</section>;
...
const ConstructorGlobal = () => {
    ...
    const { activeBook, activeChapter, chapters = [], worldItems = [], characters = [], profile, updateChapter, updateWorldItem, updateCharacter } = useData();
    ...
    const [status, setStatus] = useState(UI_STATUS[storedMission.status] || storedMission.status || 'idle');
    const [error, setError] = useState('');
    const [missionId] = useState(() => storedMission.id || `global-${Date.now()}`);
    ...
    const analyze = async () => {
        ...
        try { const nextImpact = await requestMissionImpact({ profile, mission, documents }); setImpact(nextImpact); setStep('impact'); setStatus('alternatives'); persist({ impact: nextImpact, status: 'impact_ready' }); remember({ impact: nextImpact, status: 'impact_ready' }); } catch (requestError) { setStatus('error'); setError(requestError?.message || 'No se pudo analizar el impacto.'); persist({ status: 'failed' }); }
    };
    const proposeAlternatives = async () => {
        setStatus('analyzing'); setError('');
        try { const next = await requestMissionAlternatives({ profile, mission, documents, impact }); setAlternatives(next.alternatives); setAlternativeId(next.alternatives[0]?.id || null); setStep('alternatives'); setStatus('alternatives'); persist({ alternatives: next.alternatives, alternativeId: next.alternatives[0]?.id || null, status: 'alternatives_ready' }); } catch (requestError) { setStatus('error'); setError(requestError?.message || 'No se pudieron generar alternativas.'); }
    };
    const generatePlan = async () => {
        ...
        try { const next = await requestMissionOperations({ profile, mission, documents, impact, alternative }); const prepared = { ...next, operations: prepareOperations(next.operations, documents) }; setResult(prepared); setApprovedIds(prepared.operations.filter((operation) => ['patch', 'replace', 'delete'].includes(operation.action)).map((operation) => operation.id)); setStep('plan'); setStatus('review'); persist({ result: prepared, approvedIds: prepared.operations.filter((operation) => ['patch', 'replace', 'delete'].includes(operation.action)).map((operation) => operation.id), status: 'plan_ready' }); remember({ result: prepared, status: 'plan_ready' }); } catch (requestError) { setStatus('error'); setError(requestError?.message || 'No se pudo construir el plan.'); }
    };
    ...
    const reset = () => { setStep('type'); setImpact(null); setAlternatives([]); setAlternativeId(null); setResult(null); setVerification(null); setApprovedIds([]); setError(''); setStatus('idle'); updateProcess('global-constructor', { id: null, status: 'draft', request: '', context: {}, impact: null, alternatives: [], selectedAlternativeId: null, operations: [], approvedOperationIds: [], verification: null, result: null, error: null }); };
    ...
    return <ToolRoomShell room={room} status={busy ? 'processing' : verification ? 'saved' : 'ready'} context={<div className="flex items-center gap-3"><ShieldCheck size={17} className="text-indigo-500" /><span>{statusLabel[status] || statusLabel.idle} · {documents.length} documentos de contexto</span></div>}>
        <div className="mx-auto max-w-6xl"><section className="rounded-3xl border border-indigo-500/20 bg-gradient-to-br from-indigo-500/10 via-[var(--bg-editor)] to-[var(--bg-editor)] p-6 lg:p-8"><div className="flex flex-wrap items-start justify-between gap-5"><div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-indigo-500">Cambios de alto impacto</p><h2 className="mt-2 text-3xl font-serif font-black">Constructor Global</h2><p className="mt-2 max-w-2xl text-sm leading-relaxed text-[var(--text-muted)]">La IA analiza y propone. Tú apruebas un plan antes de modificar el canon.</p></div><div className="rounded-2xl bg-indigo-500/10 p-3 text-indigo-500"><Sparkles size={24} /></div></div><StepBar step={step} /></section>
            ...
            {step === 'impact' && impact && <ImpactPanel impact={impact} questions={impact.questions} onContinue={proposeAlternatives} />}
            {step === 'alternatives' && <AlternativeCards alternatives={alternatives} selectedId={alternativeId} onSelect={setAlternativeId} onContinue={generatePlan} />}
