...
const ConstructorGlobal = () => {
    ...
    const hasFreshLaunch = Boolean(launch?.prompt);
    const storedMission = hasFreshLaunch ? {} : (stored.process || stored.mission || {});
    const launchSelectedIds = [...new Set([
        ...(launch?.context?.chapterIds || []),
        ...(launch?.context?.characterIds || []),
        ...(launch?.context?.worldItemIds || []),
    ].map(String))];
    ...
    const [scope, setScope] = useState(launchSelectedIds.length ? 'selected' : (storedMission.scope || 'automatic'));
    const [selectedIds, setSelectedIds] = useState(launchSelectedIds.length ? launchSelectedIds : (storedMission.selectedIds || []));
