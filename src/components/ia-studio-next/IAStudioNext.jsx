...
const capabilityCards = [
    ...
    { id: 'patch', title: 'Edición directa', description: 'Correcciones puntuales sobre el texto abierto.', icon: Wrench, color: 'text-amber-500 bg-amber-500/10' },
    { id: 'routing', title: 'Derivación inteligente', description: 'Cada solicitud compleja llega al espacio adecuado.', icon: ShieldCheck, color: 'text-emerald-500 bg-emerald-500/10' },
];
        ...
        <section className="h-full min-h-0 overflow-y-auto bg-[var(--bg-app)] text-[var(--text-main)]">
            <div className="max-w-6xl mx-auto p-5 lg:p-10">
                <div className="mt-8 rounded-3xl border border-indigo-500/20 bg-gradient-to-br from-indigo-500/10 via-[var(--bg-editor)] to-[var(--bg-editor)] p-6 lg:p-8 shadow-sm">
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6"><div className="flex items-start gap-4"><div className="w-12 h-12 rounded-2xl bg-indigo-600 text-white flex items-center justify-center shadow-lg shadow-indigo-600/20"><Bot size={24} /></div><div><p className="text-[9px] font-black uppercase tracking-[0.18em] text-indigo-500">Core IA Studio</p><h2 className="mt-1 text-2xl font-serif font-black">Tu centro de orientación</h2><p className="mt-2 max-w-xl text-sm leading-relaxed text-[var(--text-muted)]">El Core conversa, analiza y deriva cada solicitud al espacio adecuado. Las modificaciones complejas se revisan en Constructor Global.</p></div></div><button type="button" onClick={openCore} className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-black text-white shadow-lg shadow-indigo-600/20 hover:bg-indigo-500">Abrir Core Chat <ArrowRight size={16} /></button></div>
