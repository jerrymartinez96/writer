...
const WORK_MODES = [
    ...
    { id: 'create', label: 'Crear', eyebrow: 'Desarrollo narrativo', description: 'Diseña personajes, capítulos, escenas y borradores.', icon: BookOpen, accent: 'violet', route: 'toolroom:creative-studio', roomId: 'creative-studio', steps: ['Objetivo', 'Dirección', 'Escenas', 'Borrador'] },
...
const WorkflowLauncher = ({ mode, objective, setObjective, creativeFlow, setCreativeFlow, onLaunch, launchError }) => {
    ...
    return <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-3xl">
        <section className={`rounded-3xl border p-5 sm:p-7 ${accentClasses[mode.accent]}`}><div className="flex items-start gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[var(--bg-editor)] shadow-sm"><ModeIcon size={21} /></span><div><p className="text-[9px] font-black uppercase tracking-[0.18em] opacity-80">{mode.eyebrow}</p><p className="mt-2 max-w-xl text-sm leading-relaxed text-[var(--text-muted)]">{mode.description} La tarea conservará el objetivo y las fuentes seleccionadas al abrir el flujo especializado.</p></div></div></section>
        <section className="mt-5 rounded-3xl border border-[var(--border-main)] bg-[var(--bg-editor)] p-5 sm:p-7">
            <div className="flex items-center gap-3"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-600 text-xs font-black text-white">1</span><div><h3 className="text-sm font-black">Define el objetivo</h3><p className="mt-0.5 text-xs text-[var(--text-muted)]">Una intención concreta evita regeneraciones innecesarias.</p></div></div>
            <textarea value={objective} onChange={(event) => setObjective(event.target.value)} rows={5} placeholder={mode.id === 'audit' ? 'Ej. Revisa la cronología del segundo acto y separa contradicciones de cabos abiertos…' : mode.id === 'create' ? 'Ej. Diseña el capítulo donde Elena encuentra la pista, sin revelar todavía al traidor…' : 'Ej. Elimina la escena del puerto, conserva la deuda y traslada la revelación al invernadero…'} className="mt-5 w-full resize-none rounded-2xl border border-[var(--border-main)] bg-[var(--bg-app)] p-4 text-sm leading-relaxed outline-none transition-colors placeholder:text-[var(--text-muted)] focus:border-indigo-500" />
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border-main)] pt-5"><p className="max-w-md text-[10px] leading-relaxed text-[var(--text-muted)]">IA Studio entregará el objetivo y el contexto al siguiente paso. Ningún documento se modifica al abrirlo.</p><button type="button" onClick={onLaunch} disabled={!objective.trim()} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-black text-white shadow-sm transition-colors hover:bg-indigo-500 disabled:cursor-not-allowed disabled:opacity-40">{actionLabel} <ArrowRight size={16} /></button></div>
...
const IAStudioNext = () => {
    ...
    const handleLaunch = () => {
        ...
        if (!objective || !mode.route || !mode.roomId) return;
